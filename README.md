...
| testing | TDD 워크플로우 (RED → GREEN → IMPROVE), 커버리지 80%+ | v2 |
...
| build-error-resolver | opus | 빌드/타입 에러 자동 수정 (최소 diff) | v2 |
| refactor-cleaner | sonnet | 데드코드 정리 & 코드 통합 | v2 |
| security-reviewer | opus | OWASP Top 10 보안 취약점 전문 탐지 | v2 |
| tdd-guide | opus | TDD 워크플로우 가이드 (RED → GREEN → IMPROVE) | v2 |

### Commands (슬래시 명령어)

| 이름 | 설명 | 버전 |
|------|------|:----:|
| /overview | 프로젝트 구조 분석, project-overview.md + .overview/*.md 생성 | v3 |
| /plan-from-spec | 기획 문서를 prompt_plan.md로 변환 (입력 검증 5종) | v3 |
| /plan | planner 에이전트 호출, prompt_plan.md 저장 (overview 연동) | v1 |
...
| notify | Notification | 타입별(권한요청/유휴/추가입력/완료) Windows 토스트 알림 | v3 |
...
- **`/tdd`** — RED → GREEN → IMPROVE 워크플로우. tdd-guide 에이전트 호출.
...
```
/overview → /plan-from-spec → /auto 또는 /orchestrate
```

- **`/overview`** — 프로젝트 구조를 `project-overview.md` + `.overview/*.md`로 생성. 이후 plan 시 전체 스캔 대신 관련 모듈만 탐색.
- **`/plan-from-spec`** — 기획 문서를 읽어 `prompt_plan.md`로 변환. 마일스톤 분할, 모호함 `[CLARIFY]` 처리.
...
│   ├── commands/                   # 슬래시 명령어 (12개)
│   │   ├── overview.md                  [v3] /overview
│   │   ├── plan-from-spec.md            [v3] /plan-from-spec
│   │   ├── plan.md                      [v1] /plan
...
└── catalog.json                    # 모듈 메타데이터 (모듈 36개 + 템플릿 3개)
```
