---
description: 프로젝트 구조를 분석하여 docs/overview/ 하위에 인덱스 + 모듈 파일을 생성합니다.
---

# /overview

> 코드베이스를 분석하여 프로젝트 지도를 생성.
> 인덱스(`docs/overview/project-overview.md`) + 모듈 상세(`docs/overview/modules/*.md`)의 2계층 구조.
...
### Step 2: 모듈별 상세 분석 → `docs/overview/modules/*.md`

각 모듈에 대해 `docs/overview/modules/{module-name}.md` 생성:
...
### Step 3: 인덱스 생성 → `docs/overview/project-overview.md`
...
| 모듈 | 경로 | 역할 | 상세 |
|------|------|------|------|
| auth | src/auth/ | 사용자 인증 및 세션 관리 | [docs/overview/modules/auth.md] |
| api | src/api/ | HTTP 요청 처리 및 응답 | [docs/overview/modules/api.md] |
| db | src/db/ | 데이터베이스 스키마 및 쿼리 | [docs/overview/modules/db.md] |
| utils | src/utils/ | 공통 유틸리티 | [docs/overview/modules/utils.md] |
...
1. 기존 `docs/overview/project-overview.md` 읽기
2. `git diff --name-only` 로 마지막 갱신 이후 변경된 파일 식별
3. 변경된 파일이 속한 모듈의 `docs/overview/modules/{module}.md`만 재생성
4. 인덱스의 해당 모듈 행 갱신
...
1. 지정한 모듈의 `docs/overview/modules/auth.md`만 재분석/재생성
2. 인덱스의 해당 행도 갱신
...
/plan "인증에 2FA 추가"
  ↓
1. docs/overview/project-overview.md 읽기 (인덱스)
  ↓
2. "인증" → auth 모듈 매칭
  ↓
3. docs/overview/modules/auth.md 읽기 (상세)
  ↓
4. auth.md의 파일 목록에서 관련 파일만 Read (login.ts, token.ts)
  ↓
5. 필요 시 의존 모듈 (docs/overview/modules/db.md) 추가 로드
```

## docs/overview/project-overview.md 없을 때 (다른 명령어에서)
...
- 인덱스(`docs/overview/project-overview.md`)는 모듈당 1행 요약 — 전체를 빠르게 훑는 용도
- 상세(`docs/overview/modules/*.md`)는 모듈당 1파일 — 해당 모듈을 깊이 이해하는 용도
- 의존성은 내부 모듈 간 관계만 (외부 패키지 제외)
- 프로젝트 개요, 기술 스택은 CLAUDE.md 영역이므로 overview에 포함하지 않음
- `docs/overview/` 디렉토리는 `.gitignore`에 추가 권장 (생성된 파일이므로)
