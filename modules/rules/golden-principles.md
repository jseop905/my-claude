...
> 11 core principles. 모든 에이전트와 작업의 행동 기준.

## 1. 불변성 (Immutability)

**왜?** 변이(mutation)는 버그의 온상. 어디서 값이 바뀌었는지 추적이 불가능해진다.

**어떻게?** 새 객체/데이터를 생성하라. 원본을 직접 수정하지 마라.

## 2. 시크릿 환경 변수화

**왜?** 하드코딩된 시크릿은 커밋 한 번으로 영구 노출된다. git history에서 완전 삭제는 사실상 불가능.

**어떻게?** 환경 변수로만 접근하고, 미설정 시 즉시 에러를 발생시켜라.
...
**어떻게?** 200-400줄 정상 범위, 400줄 초과 시 분리 검토 의무, 800줄 절대 최대. 함수 50줄, 중첩 4단계가 한계. 넘으면 분리하라. (상세: `coding-style.md` 파일 구조 섹션)

## 6. 시스템 경계에서 검증

**왜?** 내부 코드는 신뢰해도 되지만, 사용자 입력과 외부 API 응답은 신뢰할 수 없다.

**어떻게?** 스키마 기반 검증으로 입력을 확인하고, 파라미터화된 쿼리로 인젝션을 차단하라.
...
**왜?** 컨텍스트 윈도우가 포화되면 초기 지시를 잊고, 품질이 급격히 저하된다.

**어떻게?** 작업은 컨텍스트의 50% 이내에서 완료하라. 큰 작업은 단계별로 나누어 새 세션에서 실행하라.

## 9. HARD-GATE: 설계 없는 코딩 금지

**왜?** "빠른 구현"을 합리화하여 설계 없이 코딩을 시작하면 방향 틀린 구현, 전체 재작성으로 이어진다.
