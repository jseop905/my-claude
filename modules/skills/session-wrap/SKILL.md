...
[Phase 0] 컨텍스트 수집
  ...
  └─ $SESSION_WRAP_DIR/context.md 생성

[Phase 1] 병렬 4개 subagent (Explore, sonnet)
  ├─ doc-updater        → 문서 업데이트 필요 곳 탐지
  ├─ automation-scout   → 반복 패턴 발견 + 자동화 후보 제안
  ├─ learning-extractor → 배운 점 추출
...
Windows 대응을 위해 `/tmp` 대신 환경변수 기반 경로를 사용한다:

```bash
SESSION_WRAP_DIR="${TEMP:-${TMP:-${HOME}/.claude/tmp}}/session-wrap"
mkdir -p "$SESSION_WRAP_DIR/results"
```
...
세션 중 변경사항을 수집하여 subagent에 전달할 컨텍스트를 구성한다.
...
if git -C "$WORK_DIR" rev-parse --is-inside-work-tree 2>/dev/null; then
  git -C "$WORK_DIR" diff --stat HEAD~5..HEAD > "$SESSION_WRAP_DIR/git-changes.txt" 2>/dev/null || true
  git -C "$WORK_DIR" diff --name-only HEAD~5..HEAD > "$SESSION_WRAP_DIR/changed-files.txt" 2>/dev/null || true
  git -C "$WORK_DIR" log --oneline -10 > "$SESSION_WRAP_DIR/recent-commits.txt" 2>/dev/null || true
fi
```
...
각 subagent 프롬프트는 `references/` 디렉터리에 위치:

| Subagent | 프롬프트 파일 | 출력 파일 |
|----------|-------------|----------|
| doc-updater | `references/prompt-doc-updater.md` | `$SESSION_WRAP_DIR/results/doc-updates.json` |
| automation-scout | `references/prompt-automation-scout.md` | `$SESSION_WRAP_DIR/results/automation-patterns.json` |
| learning-extractor | `references/prompt-learning-extractor.md` | `$SESSION_WRAP_DIR/results/learning-points.json` |
| followup-suggester | `references/prompt-followup-suggester.md` | `$SESSION_WRAP_DIR/results/followup-tasks.json` |
...
각 subagent는 결과를 JSON 파일로 `$SESSION_WRAP_DIR/results/`에 기록한다.

### 출력 JSON 스키마

모든 subagent의 출력은 동일한 아이템 스키마를 따른다 (상세: `references/output-schema.md`):
...
출력: `$SESSION_WRAP_DIR/results/merged-actions.json`
...
### 사용자 선택 필요 (user) - M건
3. [ ] [scout] "API 에러 처리" 패턴 → 자동화 후보
4. [ ] [followup] auth 모듈 리팩토링 제안
5. [ ] [learning] "RLS 패턴" 학습 포인트 기록
...
| scout 자동화 후보 | `$SESSION_WRAP_DIR/skill-candidates.md`에 기록 |
| learning 학습 포인트 | `$SESSION_WRAP_DIR/learning-points.md`에 기록 |
| followup 작업 | `$SESSION_WRAP_DIR/session-wrap-followups.md`에 기록 |
