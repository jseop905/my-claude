...
- `$SESSION_WRAP_DIR/results/doc-updates.json`
- `$SESSION_WRAP_DIR/results/automation-patterns.json`
- `$SESSION_WRAP_DIR/results/learning-points.json`
- `$SESSION_WRAP_DIR/results/followup-tasks.json`
...
최종 카테고리를 확정하고, Phase 3에서 사용자에게 보여줄 설명을 각 항목에 추가한다.

- **auto**: 위험이 없고 자동 실행 가능
  - 타임스탬프 갱신, info 수준의 기록
  - 사용자 표시: "자동 실행 (확인 불필요)" 헤더 아래 나열
- **user**: 사용자 판단이 필요
  - 코드/문서 수정 제안, 고우선순위 후속 작업
  - 사용자 표시: "선택 필요" 헤더 아래 체크박스로 나열
- **info**: 참고만 하면 되는 정보
  - 세션 통계, 낮은 중요도 학습 포인트
  - 사용자 표시: "참고 정보" 헤더 아래 나열 (선택 불가)

각 항목에 `user_display` 필드를 추가한다:
...
반드시 `$SESSION_WRAP_DIR/results/merged-actions.json`에 기록:
...
- 제거 사유를 `duplicates_removed` 카운트로 반영한다.
